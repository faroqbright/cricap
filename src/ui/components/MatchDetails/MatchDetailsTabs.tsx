...
import { getMatchHighLigtsAPI } from "@/api/methods/auth";
...
export const MatchDetailsHighlightTab: React.FC<MatchDetailsHighlightTabProps> = () => {
  ...
  useEffect(() => {
    const fetchHighlights = async () => {
      try {
        setLoading(true);
        const response = await getMatchHighLigtsAPI(id);
        setHighlights(response?.data?.highlights || []);
      } catch (err) {
        setError("Failed to fetch match highlights. Please try again later.");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (highlights.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center w-full h-full text-center">
        <img src="/assets/imgs/bg/soon.png" alt="Coming Soon" className="w-1/2 max-w-sm" />
        <p className="mt-4 text-lg font-semibold text-gray-700">No highlights available at the moment. Stay tuned!</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      <h1 className="text-3xl font-bold text-center text-gray-800 mb-8">Match Highlights</h1>

      <div className="grid grid-cols-1 gap-6">
        {highlights.map((highlight, index) => {
          const firstPart = highlight.commText.split(" ")[0];
          const remainingText = highlight.commText.slice(firstPart.length).trim();

          const isFirstPartNumeric = !isNaN(Number(firstPart));
          ...
          return (
            ...
            >
              <p className="text-gray-700 text-sm flex items-center justify-start space-x-3">
                {isFirstPartNumeric ? (
                  <span className="font-bold text-green-500 text-pretty">{firstPart}</span>
                ) : (
